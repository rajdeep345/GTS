...
def eval(model, dataset, args):
	...
	with torch.no_grad():
		...
		precision, recall, f1, senti_acc = utils.score_uniontags(args, predictions, labels, all_lengths, ignore_index=-1)
		...
		print('Sentiment Accuracy:\t{:.5f}'.format(senti_acc))
...
def test(args, general_embedding, domain_embedding):
	...
	print('Results on test sentences with single triplet:')
	sentence_packs = json.load(open(args.prefix + args.dataset + '/test.single.json'))
	instances = load_data_instances(sentence_packs, word2index, args)
	testset = DataIterator(instances, args)
	eval(model, testset, args)

	print('Results on test sentences with multiple triplets:')
	sentence_packs = json.load(open(args.prefix + args.dataset + '/test.mult.json'))
	instances = load_data_instances(sentence_packs, word2index, args)
	testset = DataIterator(instances, args)
	eval(model, testset, args)

	print('Results on test sentences with contrasting triplets:')
	sentence_packs = json.load(open(args.prefix + args.dataset + '/test.contrast.json'))
	instances = load_data_instances(sentence_packs, word2index, args)
	testset = DataIterator(instances, args)
	eval(model, testset, args)

	print('Results on test sentences with overlapping triplets:')
	sentence_packs = json.load(open(args.prefix + args.dataset + '/test.overlap.json'))
	instances = load_data_instances(sentence_packs, word2index, args)
	testset = DataIterator(instances, args)
	eval(model, testset, args)
...
if __name__=='__main__':
	...
	parser.add_argument('--class_num', type=int, default=4,
						help='label number')
	parser.add_argument('--emb', type=str, default="gen",
						help='embedding type (gen/domain-specific)')
