...
def score_uniontags(args, predicted, golden, lengths, ignore_index=-1):
	...
	for i in range(len(golden)):
		...
		# print('Predicted Tuples')
		for pair in predicted_tuple:
			# print(str(i) + '-' + '-'.join(map(str, pair)))
			p_set.add(str(i) + '-' + '-'.join(map(str, pair)))
			predicted_set.add(str(i) + '-' + '-'.join(map(str, pair)))

		for g in g_set:
			g_asp_opin = '-'.join(g.strip().split('-')[:-1])
			g_senti = g.strip().split('-')[-1]
			# print(f'g_asp_opin: {g_asp_opin}, g_senti: {g_senti}')
			for p in p_set:
				p_asp_opin = '-'.join(p.strip().split('-')[:-1])
				p_senti = p.strip().split('-')[-1]
				# print(f'p_asp_opin: {p_asp_opin}, p_senti: {p_senti}')
				if g_asp_opin == p_asp_opin:
					proper_positions += 1
					# print('Proper Positions')
					if g_senti == p_senti:
						proper_senti_count += 1
						# print('Proper Sentiment')

	correct_num = len(golden_set & predicted_set)
	precision = correct_num / len(predicted_set) if len(predicted_set) > 0 else 0
	recall = correct_num / len(golden_set) if len(golden_set) > 0 else 0
	f1 = 2 * precision * recall / (precision + recall) if (precision + recall) > 0 else 0
	senti_acc = 0 if proper_positions == 0 else float(proper_senti_count/proper_positions)
	return precision, recall, f1, senti_acc